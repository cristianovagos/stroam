...
def subscribe(user_id=1, channel_name='stroam-general'):
    if not channel_exists(channel_name):
        ...
        ch = Notification_Channels.objects.filter(channel_name=channel_name)
    ...
    if not is_user_subscribed(user_id, channel_name):
        ...
        sub.save()
        print('Subscribe - user ' + user_id + ' is now subscribed to channel ' + channel_name + ' !')
    print('Subscribe - user ' + user_id + ' is now unsubscribed to channel ' + channel_name + ' !')

def unsubscribe(user_id=1, channel_name='stroam-general'):
    if not is_user_subscribed(user_id, channel_name):
        print('Unsubscribe - user ' + user_id + ' is not subscribed to channel ' + channel_name + ', exiting...')
        return
    Notification_Subscription.objects.filter(user_id=user_id, channel_id__channel_name=channel_name).delete()
    print('Unsubscribe - user ' + user_id + ' is now unsubscribed to channel ' + channel_name + ' !')
